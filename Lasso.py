...
print('______________Lasso Regressiom__________')
LassoPrediction = Lassoreg.predict(X_test)
# approximate the result and put it in a list
LassoPrediction = list(map(round, LassoPrediction))
df_submission = pd.read_csv('sample_submission.csv')
df_submission['item_cnt_month'] = LassoPrediction
df_submission.to_csv('LassoPrediction.csv', index=False)
#make the no of rows less to get the accuracy between the y test and the y predict
LassoPrediction = LassoPrediction[0:84825]
print ('Accuracy : ', accuracy_score(y_test,LassoPrediction))
