...
print(y_train.shape)
print(y_test.shape)
print('_________GradientBoostingRegressor_______')
Gradient = GradientBoostingRegressor(
    n_estimators=100
)
Gradient.fit(x_train, y_train)
GradientPrediction = Gradient.predict(X_test)
# approximate the result and put it in a list
GradientPrediction = list(map(round, GradientPrediction))
df_submission = pd.read_csv('sample_submission.csv')
df_submission['item_cnt_month'] = GradientPrediction
df_submission.to_csv('GradientPrediction.csv', index=False)
#make the no of rows less to get the accuracy between the y test and the y predict
GradientPrediction = GradientPrediction[0:84825]
print ('Accuracy : ', accuracy_score(y_test,GradientPrediction))
